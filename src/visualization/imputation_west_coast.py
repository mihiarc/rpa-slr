"""
Script to visualize West Coast imputation coverage using choropleth maps.
"""
...
def plot_west_coast_coverage(
    imputation_file: Path,
    output_path: Path):
    """Create a choropleth map of West Coast imputation coverage.
    ...
    with open(CONFIG_DIR / "region_mappings.yaml") as f:
        region_config = yaml.safe_load(f)['regions']['west_coast']
    
    with open(CONFIG_DIR / "tide_stations" / "west_coast_tide_stations.yaml") as f:
        station_config = yaml.safe_load(f)
    
    # Load data
    print("Loading data...")
    
    # Load West Coast imputation data
    print("\nLoading West Coast imputation data...")
    imputation_df = pd.read_parquet(imputation_file)
    print(f"West Coast imputation records: {len(imputation_df)}")
    ...
    # Filter counties to West Coast states and coastal counties
    region_counties = counties[
        counties['STATEFP'].isin(west_coast_states.keys()) &
    ...
    for state_fips, count in state_counts.items():
        print(f"{west_coast_states[state_fips]}: {count} counties")
    ...
    projection = ccrs.AlbersEqualArea(
        central_longitude=-124.0,  # Centered on West Coast
        central_latitude=42.0,     # Centered on Oregon coast
        standard_parallels=(36.0, 48.0)  # From CA to WA
    )
    
    # Set figure style
    plt.style.use('seaborn-v0_8')
    
    # Create figure with taller aspect ratio for West Coast
    fig = plt.figure(figsize=(12, 18))  # Adjusted for north-south coastline
    ...
    for station_id, station_info in station_config['stations'].items():
        ...
        lon_offset = 0.05
        if station_info['location']['lon'] < -124:  # For stations far west
            lon_offset = -0.4  # Place text to the left
            ha = 'right'
        else:
            ha = 'left'
    ...
    title = (
        f"West Coast Tide Gauge Coverage\n"
    ...
    caption = (
        "Tide gauge coverage assessment for West Coast counties. "
