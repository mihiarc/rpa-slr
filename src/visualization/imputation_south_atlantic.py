"""
Script to visualize South Atlantic imputation coverage using choropleth maps.
"""
...
def plot_south_atlantic_coverage(
    imputation_file: Path,
    output_path: Path):
    """Create a choropleth map of South Atlantic imputation coverage.
    ...
    with open(CONFIG_DIR / "region_mappings.yaml") as f:
        region_config = yaml.safe_load(f)['regions']['south_atlantic']
    
    with open(CONFIG_DIR / "tide_stations" / "south_atlantic_tide_stations.yaml") as f:
        station_config = yaml.safe_load(f)
    
    # Load data
    print("Loading data...")
    
    # Load South Atlantic imputation data
    print("\nLoading South Atlantic imputation data...")
    imputation_df = pd.read_parquet(imputation_file)
    print(f"South Atlantic imputation records: {len(imputation_df)}")
    
    # Load counties from main county file and filter for South Atlantic states
    print("\nLoading county geometries...")
    counties = gpd.read_parquet(PROCESSED_DATA_DIR / "county.parquet")
    
    # Get South Atlantic state FIPS codes
    south_atlantic_states = {'12': 'FL', '13': 'GA', '37': 'NC', '45': 'SC'}
    
    # Filter counties to South Atlantic states and coastal counties
    region_counties = counties[
        counties['STATEFP'].isin(south_atlantic_states.keys()) &
    ...
    for state_fips, count in state_counts.items():
        print(f"{south_atlantic_states[state_fips]}: {count} counties")
    ...
    projection = ccrs.AlbersEqualArea(
        central_longitude=-78.5,  # Centered on South Atlantic coast
        central_latitude=31.0,    # Centered between FL and NC
        standard_parallels=(29.0, 33.0)
    )
    ...
    title = (
        f"South Atlantic Tide Gauge Coverage\n"
    ...
    caption = (
        "Tide gauge coverage assessment for South Atlantic counties. "
...
def main():
    """Generate South Atlantic imputation coverage visualization."""
    # Find most recent South Atlantic imputation file
    imputation_files = list(IMPUTATION_OUTPUT_DIR.glob("imputation_structure_south_atlantic_*.parquet"))
    if not imputation_files:
        print("No South Atlantic imputation structure files found")
        return
    ...
    output_path = output_dir / "south_atlantic_coverage.png"
