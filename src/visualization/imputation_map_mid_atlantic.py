"""
Script to visualize Mid Atlantic imputation coverage using choropleth maps.
"""
...
def plot_mid_atlantic_coverage(
    imputation_file: Path,
    output_path: Path):
    """Create a choropleth map of Mid Atlantic imputation coverage.
    ...
    with open(CONFIG_DIR / "region_mappings.yaml") as f:
        region_config = yaml.safe_load(f)['regions']['mid_atlantic']
    
    with open(CONFIG_DIR / "tide_stations" / "mid_atlantic_tide_stations.yaml") as f:
        station_config = yaml.safe_load(f)
    
    # Load data
    print("Loading data...")
    
    # Load Mid Atlantic imputation data
    print("\nLoading Mid Atlantic imputation data...")
    imputation_df = pd.read_parquet(imputation_file)
    print(f"Mid Atlantic imputation records: {len(imputation_df)}")
    ...
    # Filter counties to Mid Atlantic states and coastal counties
    region_counties = counties[
        counties['STATEFP'].isin(mid_atlantic_states.keys()) &
    ...
    for state_fips, count in state_counts.items():
        print(f"{mid_atlantic_states[state_fips]}: {count} counties")
    ...
    projection = ccrs.AlbersEqualArea(
        central_longitude=-75.0,  # Centered on Mid Atlantic coast
        central_latitude=39.0,    # Centered between VA and NY
        standard_parallels=(37.0, 41.0)
    )
    ...
    title = (
        f"Mid Atlantic Tide Gauge Coverage\n"
    ...
    caption = (
        "Tide gauge coverage assessment for Mid Atlantic counties. "
