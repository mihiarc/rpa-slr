"""
Script to visualize North Atlantic imputation coverage using choropleth maps.
"""
...
def plot_north_atlantic_coverage(
    imputation_file: Path,
    output_path: Path):
    """Create a choropleth map of North Atlantic imputation coverage.
    ...
    with open(CONFIG_DIR / "region_mappings.yaml") as f:
        region_config = yaml.safe_load(f)['regions']['north_atlantic']
    
    with open(CONFIG_DIR / "tide_stations" / "north_atlantic_tide_stations.yaml") as f:
        station_config = yaml.safe_load(f)
    
    # Load data
    print("Loading data...")
    
    # Load North Atlantic imputation data
    print("\nLoading North Atlantic imputation data...")
    imputation_df = pd.read_parquet(imputation_file)
    print(f"North Atlantic imputation records: {len(imputation_df)}")
    
    # Load counties from main county file and filter for North Atlantic states
    print("\nLoading county geometries...")
    counties = gpd.read_parquet(PROCESSED_DATA_DIR / "county.parquet")
    
    # Get North Atlantic state FIPS codes
    north_atlantic_states = {'09': 'CT', '23': 'ME', '25': 'MA', '33': 'NH', '44': 'RI'}
    
    # Filter counties to North Atlantic states and coastal counties
    region_counties = counties[
        counties['STATEFP'].isin(north_atlantic_states.keys()) &
    ...
    for state_fips, count in state_counts.items():
        print(f"{north_atlantic_states[state_fips]}: {count} counties")
    ...
    projection = ccrs.AlbersEqualArea(
        central_longitude=-70.0,  # Centered on North Atlantic coast
        central_latitude=43.0,    # Centered on Maine coast
    ...
    title = (
        f"North Atlantic Tide Gauge Coverage\n"
    ...
    caption = (
        "Tide gauge coverage assessment for North Atlantic counties. "
...
def main():
    """Generate North Atlantic imputation coverage visualization."""
    # Find most recent North Atlantic imputation file
    imputation_files = list(IMPUTATION_OUTPUT_DIR.glob("imputation_structure_north_atlantic_*.parquet"))
    if not imputation_files:
        print("No North Atlantic imputation structure files found")
        return
    ...
    output_path = output_dir / "north_atlantic_coverage.png"
